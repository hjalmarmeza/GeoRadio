...
export async function getStations(countryName, cityName, limit = 100) {
    // Fetch up to 1000 stations for the country to ensure we catch all cities/states
    const endpoint = `/stations/search?country=${encodeURIComponent(countryName)}&hidebroken=true&order=clickcount&reverse=true&limit=1000`;
    ...
    if (cityName) {
        stations = stations.filter(station => {
            let sCity = station.city || station.state || '';
            sCity = sCity.trim();
            if (sCity.length > 2) {
                sCity = sCity.charAt(0).toUpperCase() + sCity.slice(1);
            }
            // Strict match to the formatted city name generated by getCities
    ...
    return enrichWithWhatsApp(stations);
}
